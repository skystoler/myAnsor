# copied from https://github.com/tensorflow/models/blob/master/official/resnet/resnet_model.py
# this version is the same as in tvm benchmark
...
def batch_norm_relu(inputs, is_training, data_format):
  ...
  inputs = tf.compat.v1.layers.batch_normalization(
      inputs=inputs, axis=1 if data_format == 'channels_first' else 4,
      momentum=_BATCH_NORM_DECAY, epsilon=_BATCH_NORM_EPSILON, center=True,
      scale=True, training=is_training, fused=True)
  #inputs = tf.keras.layers.BatchNormalization(
  #    axis=1 if data_format == 'channels_first' else 4,
  #    momentum=_BATCH_NORM_DECAY, epsilon=_BATCH_NORM_EPSILON, center=True,
  #    scale=True, trainable=is_training)(inputs)
  inputs = tf.nn.relu(inputs)
  return inputs


def fixed_padding(inputs, kernel_sizes, data_format):
  """Pads the input along the spatial dimensions independently of input size.

  Args:
    inputs: A tensor of size [batch, channels, height_in, width_in] or
      [batch, height_in, width_in, channels] depending on data_format.
    kernel_size: The kernel to be used in the conv3d or max_pool3d operation.
  ...
  for kernel_size in kernel_sizes:
    ...
    pad_begs.append(pad_total // 2)
    pad_ends.append(pad_total - pad_begs[-1])

  if data_format == 'channels_first':
    padded_inputs = tf.pad(inputs, [[0, 0],
                                    [0, 0],
                                    [pad_begs[0], pad_ends[0]],
                                    [pad_begs[1], pad_ends[1]],
                                    [pad_begs[2], pad_ends[2]]])
  else:
    padded_inputs = tf.pad(inputs, [[0, 0],
                                    [pad_begs[0], pad_ends[0]],
                                    [pad_begs[1], pad_ends[1]],
...
def conv3d_fixed_padding(inputs, filters, kernel_size, strides, data_format):
  ...
  # dimensions of `inputs` (as opposed to using `tf.compat.v1.layers.conv3d` alone).
...
def imagenet_resnet_v2_generator(block_fn, layers, num_classes,
  ...
  def model(inputs, is_training):
    ...
    inputs = conv3d_fixed_padding(
        inputs=inputs, filters=64, kernel_size=(3, 7, 7), strides=[1, 2, 2],
        data_format=data_format)
    inputs = tf.identity(inputs, 'initial_conv')
    #inputs = tf.compat.v1.layers.max_pooling3d(
    ...
    inputs = batch_norm_relu(inputs, is_training, data_format)
    inputs = tf.keras.layers.AveragePooling3D(
        pool_size=[2, 7, 7], strides=1, padding='VALID', data_format=data_format)(inputs)
