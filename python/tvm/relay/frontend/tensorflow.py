...
def _power():
    def _impl(inputs, attr, params, mod):
        print("relay.frontend.tensorflow: Power input", inputs[1].data.asnumpy())
        if (inputs[1].data.asnumpy() == 3.0):
            mul = get_relay_op("multiply")
            return mul(mul(inputs[0], inputs[0]), inputs[0])
...
def _softmax():
    def _impl(inputs, attr, params, mod):
        return AttrCvt(op_name='fast_softmax',
