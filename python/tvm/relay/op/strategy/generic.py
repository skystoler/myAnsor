...
def wrap_compute_conv2d(topi_compute, need_data_layout=False, need_out_layout=False,
    ...
    def _compute_conv2d(attrs, inputs, out_type):
        ...
        #if (topi_compute.__name__ == "conv2d_nhwc" and type(attrs) == op.op_attrs.Conv2DAttrs):
        ansor_kernel_layout = attrs.get_str("ansor_kernel_layout")
        args.append(ansor_kernel_layout)
    ...
    def _compute_conv3d(attrs, inputs, out_type):
        ...
        if need_layout:
            out = topi_compute(inputs[0], inputs[1], strides, padding, dilation,
                               layout, out_dtype, ansor_kernel_layout)
        else:
            out = topi_compute(inputs[0], inputs[1], strides, padding, dilation,
                               out_dtype, ansor_kernel_layout)
        return [out]
...
def wrap_compute_dense(topi_compute):
    ...
    def _compute_dense(attrs, inputs, out_type):
        ...
        ansor_kernel_layout = attrs.get_str("ansor_kernel_layout")
        return [topi_compute(inputs[0], inputs[1], None, out_dtype, ansor_kernel_layout)]
...
def wrap_compute_batch_matmul(topi_compute):
    ...
    def _compute_batch_matmul(attrs, inputs, out_type):
        ...
        ansor_kernel_layout = attrs.get_str("ansor_kernel_layout")
        return [topi_compute(inputs[0], inputs[1], weight_transposed, ansor_kernel_layout)]
