...
def depthwise_conv2d_nhwc(Input, Filter, stride, padding, dilation, out_dtype=None, ansor_kernel_layout=""):
    ...
    if ansor_kernel_layout:
        ...
        if len(Filter.shape) >= 8:
            ...
            filter_channel = Filter.shape[0] * Filter.shape[1] * Filter.shape[4]
            channel_multiplier = 1
        elif len(Filter.shape) == 4:
            # For cpu tile structure SRS
            filter_height = Filter.shape[1]
            filter_width = Filter.shape[2]
            filter_channel = Filter.shape[0] * Filter.shape[3]
            channel_multiplier = 1
