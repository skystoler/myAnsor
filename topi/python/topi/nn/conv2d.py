...
def conv2d_nhwc(Input, Filter, stride, padding, dilation, out_dtype='float32', ansor_kernel_layout=""):
    ...
    if ansor_kernel_layout:
        ...
        if len(Filter.shape) >= 10:
            ...
            raise ValueError("Don't know how to infer the layout for filter shape: %s. " \
    ...
    if ansor_kernel_layout:
        Output = ansor.rewrite_compute_body(Output, Filter, ansor_kernel_layout)
...
def _conv2d_nhwc_winograd_impl(input, weight, strides, padding, dilation, out_dtype, tile_size,
        pre_computed=False, ansor_kernel_layout=""):
