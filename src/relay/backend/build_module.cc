...
class RelayBuildModule : public runtime::ModuleNode {
 ...
 protected:
  ...
  IRModule Optimize(IRModule relay_module, const TargetsMap& targets,
    ...
    // Rewrite the layout of "layout free" placeholders for the auto-scheduler. 
    if (targets.size() == 1) {
      const auto& target = (*targets.begin()).second;
      Pass major_pass = transform::KernelLayoutTransform();

      relay_module = seq(relay_module);

      if (pass_ctx.PassEnabled(major_pass->Info())) {
