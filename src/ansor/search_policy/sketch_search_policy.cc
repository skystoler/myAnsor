...
void SketchSearchPolicyNode::SearchOneRound(std::vector<State>* best_states,
  ...
  State tmp_s = sketch_cache_[rand_gen_() % sketch_cache_.size()];
  ...
  StdCout(verbose) << "Sample Initial State\t#s: " << std::endl;
  std::cerr << tmp_s;
  std::cerr << "==============================================" << std::endl;
...
std::string ToTabuStr(const State& state){
	std::string res;
	std::vector<Stage> stagev=state->stages;
	for(auto s:stagev){
...
static LRUList* head=nullptr;
static std::map<std::string, LRUList*> mp;
static int idx=0;

void remove(LRUList *nd)
{
    mp.erase(nd->val);
    if (nd == nd->next) // 只有一个节点
    {
        nd = nullptr;
        return;
    }
...
void insert(LRUList *nd) // nd插入到head
{
    ...
    nd->pre = head->pre;
    nd->next = head;
    head->pre->next = nd; // 这一句必须在下一句前面，因为下一句会更改head->pre
    head->pre = nd;
    head = nd;
}

bool InTabu(std::string tabu_str){
  /*
	LRUList *lst=head;
	while(lst!=head){
		if(tabu_str.find(lst->val)) return true;
		lst=lst->next;
	}
  */
  if(mp.count(tabu_str))return true;
	return false;
}

void UpdateTabu(std::string tabu_str){
    LRUList *nd = new LRUList(tabu_str, nullptr, nullptr);
    if (idx < TABU_LENGTH) // 内存页面有剩余
    {
        if (!mp.count(tabu_str)) // nd不在内存页面中时，才追加计数
        {
            ++idx;
        }
        insert(nd);
    ...
    {
        if (!mp.count(tabu_str)) // nd不在内存页面时，才需要淘汰队尾腾出空间
        {
            remove(head->pre);
        }
        insert(nd);
...
void SketchSearchPolicyNode::EvolutionarySearch(
  ...
  if (IsGPUTask(cur_task)) {
	//rule_weights={1.00,0.00,0.00,0.00};
	//rule_weights={0.00,1.00,0.00,0.00};
 	rule_weights = {0.90, 0.10, 0.00, 0.00};
  } else {
  	rule_weights = {0.90, 0.03, 0.05, 0.02};
	//rule_weights = {1.00, 0.00, 0.00, 0.00};
	// rule_weights={0.00,1.00,0.00,0.00};
  // rule_weights={0.00,0.00,1.00,0.00};
	//rule_weights={0.00,0.00,0.00,1.00};
  }

  for(auto rule_weight:rule_weights){
    StdCout(verbose)<<rule_weight<<" ";
  }
  ...
  */
    ...
    //StdCout(verbose)<<"num_cross_over: "<<num_cross_over<<std::endl; 2048*0.05=102

    //StdCout(verbose)<<"num_cross_over_bound: "<<num_cross_over_trial_upper_bound<<std::endl;

    //StdCout(verbose)<<"pnext.size: "<<static_cast<int>(pnext->size())<<std::endl;
    while (cross_over_enabled_ &&
      ...
      if (p1 == p2 || (*pnow)[p1].ToStr() == (*pnow)[p2].ToStr()) {
          ...
          if(tmp_s.defined()) {
              ...
              std::vector<State> child_state={tmp_s};
              std::vector<State> *pcs= &child_state;
              std::vector<float> child_scores(1,0); 
              program_cost_model->Predict(cur_task, *pcs, &child_scores);
              float child_score = child_scores[0];
            
              if(child_score >= average_pa){
                  fout<<"1 ";
                  pnext->push_back(std::move(tmp_s));
              }else{
                  std::string tabu_str=ToTabuStr(tmp_s);
                  if(!InTabu(tabu_str)){
                      fout<<"2 ";
                      pnext->push_back(std::move(tmp_s));
                      UpdateTabu(tabu_str);
                      ...
                      fout<<"3 ";
                      pnext->push_back(a > b ? (*pnow)[p1] : (*pnow)[p2]);
                  }	
              }
            
              //StdCout(verbose)<<"length: "<<static_cast<int>(tmp_s->stages.size())<<std::endl;
              //pnext->push_back(std::move(tmp_s));
    ...
    double duration = std::chrono::duration_cast<std::chrono::duration<double> >(
    std::chrono::high_resolution_clock::now() - tic_begin).count();
    fout<<std::endl<<"duration: "<<std::fixed << std::setprecision(2)<<duration<<std::endl;
  }
