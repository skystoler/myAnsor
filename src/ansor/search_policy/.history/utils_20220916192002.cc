...
State CrossOverState(const SearchTask& task, std::mt19937* random_gen, const State& p1,
  ...
  //single crossover
  int seg = static_cast<int>(p1->stages.size());
  for (int t=seg-1; t >= 0; --t) {
    ...
    if((seg-t) % 3 == 0){
      if ((*random_gen)() % 100 >= proportion) {  
        stage_out_to_states[p2->stages[t]->op->name] = sync_p2.id;
        stage_out_to_states[p2->stages[t+1]->op->name] = sync_p2.id;
        stage_out_to_states[p2->stages[t+2]->op->name] = sync_p2.id;
        if (p2->stages[t]->compute_at != kInlined) {
          p2_selected++;
        }
        if (p2->stages[t+1]->compute_at != kInlined) {
          p2_selected++;
        }
        if (p2->stages[t+2]->compute_at != kInlined) {
          p2_selected++;
        }
      } else {
        stage_out_to_states[p1->stages[t]->op->name] = sync_p1.id;
        stage_out_to_states[p1->stages[t+1]->op->name] = sync_p1.id;
        stage_out_to_states[p1->stages[t+2]->op->name] = sync_p1.id;
        if (p1->stages[t]->compute_at != kInlined) {
          p1_selected++;
        }
        if (p1->stages[t+1]->compute_at != kInlined) {
          p1_selected++;
        }
        if (p1->stages[t+2]->compute_at != kInlined) {
          p1_selected++;
        }
      }
      ...
      if(t==1){
        if ((*random_gen)() % 100 >= proportion) {  
          stage_out_to_states[p2->stages[0]->op->name] = sync_p2.id;
          if (p2->stages[0]->compute_at != kInlined) {
            p2_selected++;
          }
        }else{
          stage_out_to_states[p1->stages[0]->op->name] = sync_p1.id;
          if (p1->stages[0]->compute_at != kInlined) {
            p1_selected++;
          }
        }
      ...
      if(t==2){
          ...
          stage_out_to_states[p2->stages[1]->op->name] = sync_p2.id;
          if (p2->stages[1]->compute_at != kInlined) {
            p2_selected++;
          }
        }else{
          stage_out_to_states[p1->stages[0]->op->name] = sync_p1.id;
          ...
          stage_out_to_states[p1->stages[1]->op->name] = sync_p1.id;
          if (p1->stages[1]->compute_at != kInlined) {
            p1_selected++;
          }
        }
      }
