...
def batch_norm_relu(inputs, is_training, data_format):
  ...
  #    momentum=_BATCH_NORM_DECAY, epsilon=_BATCH_NORM_EPSILON, center=True,
  #    scale=True, trainable=is_training, fused=True)(inputs)
...
def conv2d_fixed_padding(inputs, filters, kernel_size, strides, data_format):
  ...
  # dimensions of `inputs` (as opposed to using `tf.compat.v1.layers.conv2d` alone).
  if strides > 1:
...
def imagenet_resnet_v2_generator(block_fn, layers, num_classes,
  ...
  def model(inputs, is_training):
    ...
    inputs = conv2d_fixed_padding(
        ...
        data_format=data_format)
    inputs = tf.identity(inputs, 'initial_conv')
    inputs = tf.keras.layers.MaxPool2D(pool_size=3, strides=2, padding='SAME', data_format=data_format)(inputs)
    ...
    inputs = batch_norm_relu(inputs, is_training, data_format)
    inputs = tf.keras.layers.AveragePooling2D(pool_size=7, strides=1, padding='VALID',
            data_format=data_format)(inputs)
    inputs = tf.identity(inputs, 'final_avg_pool')
