...
from utils import str2bool, log_line, BenchmarkRecord, get_network
...
parser.add_argument("--target", type=str, default='llvm -target=arm-linux-gnueabihf -mattr=+neon')
parser.add_argument("--network", type=str, required=True)
parser.add_argument("--min-repeat", type=int, default=50)
parser.add_argument("--n-trials", type=int, default=1250)
parser.add_argument("--batch-size", type=int, default=1)
parser.add_argument("--num-threads", type=int)
parser.add_argument("--rpc-device-key", type=str, default='rasp4b')
parser.add_argument("--rpc-host", type=str, default='kraken')
parser.add_argument("--rpc-port", type=int, default=9191)
parser.add_argument("--rpc-n-parallel", type=int, default=1)
...
log_file = "%s-B%d.log" % (model_name, batch_size)
...
tuning_option = {
    ...
    'measure_option': autotvm.measure_option(
        ...
        runner=autotvm.RPCRunner(
            args.rpc_device_key, host=args.rpc_host, port=args.rpc_port,
            n_parallel=args.rpc_n_parallel,
            number=10, timeout=10, min_repeat_ms=args.min_repeat,
        ),
    ),
}
...
def tune_and_evaluate(tuning_opt):
    mod, params, data_name, data_shape, data_dtype, out_shape = get_network(model_name, batch_size)
    ...
    with autotvm.apply_history_best(log_file):
        ...
        print("Upload...")
        remote = autotvm.measure.request_remote(args.rpc_device_key, args.rpc_host,
                                                args.rpc_port, timeout=10000)
        remote.upload(tmp.relpath(filename))
        rlib = remote.load_module(filename)

        # upload parameters to device
        ctx = remote.context(str(target), 0)
        data_tvm = tvm.nd.array((np.random.uniform(size=data_shape)).astype(data_dtype))
        module = runtime.create(graph, rlib, ctx)
        module.set_input(data_name, data_tvm)
        ...
        log_line(BenchmarkRecord(args.rpc_device_key, 'cpu', 'network', "%s.B%d" % (model_name, args.batch_size),
                                 'AutoTVM', 'NCHWc', {"costs": prof_res / 1000}, time.time()),
                                 args.out_file)
