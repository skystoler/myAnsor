...
graph_opt_sch_file = "%s-B%d-graph-opt.log" % (model_name, batch_size)
...
def tune_kernels(tasks,
    ...
    for i, task in enumerate(tasks):
        ...
        # do tuning
        n_trial = min(len(task.config_space), args.n_trials)
...
def tune_graph(graph, data_name, data_shape, records, opt_sch_file, use_DP=True):
    target_op = [relay.op.get("nn.conv2d"),]
    Tuner = DPTuner if use_DP else PBQPTuner
    executor = Tuner(graph, {data_name: data_shape}, records, target_op, target)
...
def tune_and_evaluate(tuning_opt):
    ...
    with ctx:
        ...
        log_line(BenchmarkRecord(args.device, 'cpu', 'network', "%s.B%d" % (model_name, args.batch_size),
